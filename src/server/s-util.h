...
extern int16_t get_last_info_line(struct player *p);
...
extern void set_ghost_flag(struct player *p, int16_t flag, bool report);
extern void notify_player(struct player *p, char *header, uint16_t term, bool symbol);
extern void notify_player_popup(struct player *p, char *header, uint16_t term, uint16_t pop);
