 ...
 *
 * dice is the object the variable is being added to.
...
static int dice_add_variable(dice_t *dice, const char *name)
{
    ...
    if (dice->expressions == NULL)
    {
        dice->expressions = mem_zalloc(DICE_MAX_EXPRESSIONS * sizeof(dice_expression_entry_t));
    }
 ...
 *
 * dice is the object that will use the expression..
 * name is the variable that the expression should be bound to.
 * expression is the expression to bind.
 ...
 *
 * dice is the dice object to parse the string into.
...
void dice_random_value(const dice_t *dice, void *data, random_value *v)
{
    ...
    if (dice->ex_b)
    {
        if (dice->expressions != NULL && dice->expressions[dice->b].expression != NULL)
            v->base = expression_evaluate(dice->expressions[dice->b].expression, data);
        else
            v->base = 0;
    }
    else
    ...
    if (dice->ex_x)
    {
        if (dice->expressions != NULL && dice->expressions[dice->x].expression != NULL)
            v->dice = expression_evaluate(dice->expressions[dice->x].expression, data);
        else
            v->dice = 0;
    }
    else
    ...
    if (dice->ex_y)
    {
        if (dice->expressions != NULL && dice->expressions[dice->y].expression != NULL)
            v->sides = expression_evaluate(dice->expressions[dice->y].expression, data);
        else
            v->sides = 0;
    }
    else
    ...
    if (dice->ex_m)
    {
        if (dice->expressions != NULL && dice->expressions[dice->m].expression != NULL)
            v->m_bonus = expression_evaluate(dice->expressions[dice->m].expression, data);
        else
            v->m_bonus = 0;
    }
    else
